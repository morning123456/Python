...
def callback():
    ...
    token_request = requests.get("https://nid.naver.com/oauth2.0/token?grant_type=authorization_code&client_id=zMkOhkDYiQM_zxV99Za1&client_secret=b2oF0ZKpRL&code=code")
    token_json = token_request.json()
    print(token_json)
    
    # 결과 값에서 acccess_token(접근 토큰)을 사용하여 유저에 대한 정보를 받아온다. 토큰의 만료기간도 json값 안에 있다.
    access_token = token_json.get("access_token")
    profile_request = requests.get("https://openapi.naver.com/v1/nid/me", headers={"Authorization" : "Bearer access_token"},)
